  ...
  <!--  1  -->
    <div class="bk-card" style="display: flex;justify-content: space-between">
      <div style="display: flex">
        <div>
          话题名: <el-input v-model="condition.name" size="small" placeholder="搜索话题名" style="width: 150px"/>
        </div>
        ...
        <div style="margin-left: 10px">
          ...
          <el-select v-model="condition.orderField" placeholder="请选择" size="small" style="width: 100px">
            ...
            <el-option  key="2" label="按访问量" value="boiling_count"></el-option>
          </el-select>
        </div>
      ...
      <div>
        <el-button type="primary" icon="el-icon-plus" size="small" @click="toAddTopic">添加</el-button>
  ...
  <!--  2  -->
    <el-card class="box-card" style="margin-top: 10px;">
      <el-table height="480px" :data="tableData" style="width: 100%" highlight-current-row size="small">
        <el-table-column fixed prop="avatar" label="话题图片" width="150">
          <template slot-scope="scope">
                <img :src="scope.row.avatar"  style="width: 80px;height: 80px">
          </template>
        </el-table-column>
        <el-table-column prop="name" label="话题名" width="120"/>
        <el-table-column prop="description" show-overflow-tooltip label="简介" width="120"/>
        <el-table-column prop="focusCount" label="关注量" width="120"/>
        <el-table-column prop="boilingCount" label="访问量" width="120"/>
        ...
        <el-table-column fixed="right" label="操作" width="200">
          <template slot-scope="scope">
            <el-button @click="toUpdateTopic(scope.row)" type="text" size="small">修改</el-button>
  ...
  export default {
    ...
    methods:{
      ...
      search(){
        ...
        this.$http.topic.getPageByCondition(con).then(res => {
      ...
      addTopic(){
        ...
        this.$http.topic.saveTopic(this.updateDialogForm).then(res => {
          if (res.code === "200"){
      ...
      deleteTopic(id){
         this.$http.topic.deleteById(id).then(res => {
           if (res.code === "200"){
